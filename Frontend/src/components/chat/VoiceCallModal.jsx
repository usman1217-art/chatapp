...
function VoiceCallModal({ receiver, onClose, isCaller, incomingSignal }) {
  ...
  // Safely extract the receiver's ID (handles both string ID and populated object)
  ...
  // Free public STUN servers for WebRTC peer-to-peer routing
  const iceServers = {
    iceServers: [
  ...
  useEffect(() => {
    ...
    const startCallFlow = async () => {
      try {
        // 1. Request microphone access (Requires HTTPS or localhost)
        ...
        // Socket Listeners for connection completion
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/60 dark:bg-black/75 backdrop-blur-md p-4 animate-fade-in">
      {/* Hidden audio element to output remote audio stream */}
